...
export default function UsersPage() {
    ...
    async function handleUpdate(data) {
        ...
        try {
            const res = await UsersApi.setAuthHeader(user.token).putAsync(data);

            // Update the local user list after successful API update
            const updatedUsers = [...users]; // Make a copy of the current users
            const index = updatedUsers.findIndex(u => u.id === data.id); // Find the index of the updated user
            if (index !== -1) {
    ...
    return (
        <>
        {loading && <Loader />}
        {!loading && users.length === 0 && <h3 className="text-center">אין משתמשים</h3>}
        {!loading && <>
...
function UserForm(props) {
    const rolesArray = ["subAdmin", "member", "manager", "admin"];
